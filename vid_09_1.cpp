...
//  Parameter gain{"gain", 0.01, 0, 1};
  ...
  Parameter gain{"Gain", 1, 0, 10};


  static const int Nb = 30; // Number of boids
  // Boid boids[Nb];
  Mesh tails;
  Mesh box;
  Mesh mesh;
...
//      modulator.freq(mtof(modulation.get()));
      for (int vid = 0; vid<Nb; vid++){
        carrier[vid].freq(frequency.get() + (data[vid][frame[vid]]* modulation.get() )  );
        float v = carrier[vid]() * dbtoa(gain.get());
        sounda[vid] = v * 0.0001;
        // cout<< "    " << v << "         " <<sounda[vid] << endl;
      }
      for (int vid = 0; vid<Nb; vid++){
        asdf += sounda[vid];
      }
      io.out(0) = io.out(1) = asdf;
      // cout << frame[0] << "   " << frame[9] << "    " << asdf << endl;
      // cout << i << "  " << k << "    " <<  asdf << endl;
    }
  }
