...
// Shared Parameters
int covi;
float shared_freq[Nb];
float shared_amp[Nb];
// Synth para
float attackTime[Nb];
float releaseTime[Nb];
float sustain[Nb];
float idx1[Nb], idx2[Nb], idx3[Nb];
float carMul[Nb], modMul[Nb];
float vibRate1[Nb], vibRate2[Nb], vibRise[Nb], vibDepth[Nb], pan[Nb];
...
struct Line
{
  ...
  void set()
  {
    increment = (target - value) / (ap::sampleRate * (milliseconds / 1000.0f));
  }
